"""Assemble the LangGraph pipeline.
    ...
    START -> summarize -> {bug, security, test} (parallel) -> orchestrate -> END

The three specialist agents fan out from ``summarize`` (one parallel superstep, each
reading the shared summary) and fan in to ``orchestrate``, which runs only once all three
have completed. Agents append to ``findings`` via the additive reducer on the state.
"""
...
def review_diff(diff: str, context: str = "") -> dict:
    ...
    app = build_graph()
    return app.invoke({"diff": diff, "context": context, "findings": []})
